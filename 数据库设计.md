# 校园一卡通系统数据库设计（OpenGauss）

## 一、数据库概述

本系统基于 OpenGauss 数据库设计，实现校园一卡通系统的完整业务功能。数据库交付以根目录 `init.sql` 为唯一初始化脚本，初始化对象位于 `campus_card` schema，本文档中的表结构、约束、索引和初始化数据均按 `init.sql` 整理，不依赖其他增量脚本。

系统按照模块划分进行数据库设计，涵盖：

- 核心业务模块
- 辅助业务模块
- 系统管理模块

数据库采用：

- 逻辑外键（不使用物理外键）
- 软删除机制
- 全流程数据记录

---
## 数据库表总览

| 表名               | 说明           |
|--------------------|----------------|
| student            | 学生信息表     |
| teacher            | 教师信息表     |
| college_major      | 学院专业字典表 |
| admin_user         | 管理员表       |
| campus_card        | 校园卡表       |
| card_change_record | 校园卡操作记录表 |
| account            | 账户表         |
| account_flow       | 账户流水表     |
| recharge_record    | 充值记录表     |
| recharge_order     | 支付宝充值订单表 |
| consume_record     | 消费记录表     |
| merchant           | 商户表         |
| merchant_type      | 商户类型表     |
| product            | 商品表         |
| access_point       | 门禁点表       |
| access_record      | 门禁记录表     |
| book               | 图书表         |
| borrow_application | 借阅申请表     |
| borrow_record      | 借阅记录表     |
| attendance_location | 打卡位置表     |
| attendance_record  | 考勤记录表     |
| attendance_application | 考勤申报表 |
| commute_route      | 通勤车线路表   |
| commute_vehicle    | 通勤车车辆表   |
| commute_station    | 通勤车站点表   |
| commute_schedule   | 通勤车时刻表   |
| commute_record     | 通勤车记录表   |
| operation_log      | 系统操作日志表 |

## 二、设计规范

### 2.1 逻辑外键

所有关联关系使用 `xxx_id` 维护，不使用物理外键约束，数据一致性由业务层和事务控制。典型关联包括：

- `campus_card.user_id` + `user_type` 关联学生或教师。
- `account.card_id` 关联校园卡。
- `recharge_record.account_id`、`consume_record.account_id`、`account_flow.account_id` 关联账户。
- `attendance_location.teacher_id` 关联发布打卡位置的教师。
- `attendance_application.student_id`、`card_id` 关联申报学生和校园卡。
- `commute_record.route_id`、`vehicle_id`、`schedule_id` 关联通勤车基础数据。

---

### 2.2 软删除

统一字段：

```sql
is_deleted INT DEFAULT 0
````

---

### 2.3 通用字段

```sql
create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
update_time TIMESTAMP,
is_deleted INT DEFAULT 0
```

---

## 三、核心业务模块

---

### 3.1 用户管理

#### student 学生表

```sql
CREATE TABLE student (
    id BIGSERIAL PRIMARY KEY,
    student_no VARCHAR(20) NOT NULL,
    name VARCHAR(50),
    gender VARCHAR(10),
    college VARCHAR(100),
    major VARCHAR(100),
    class_name VARCHAR(50),
    phone VARCHAR(20),
    teacher_id BIGINT,
    attendance_mode VARCHAR(20) DEFAULT 'CAMPUS',
    attendance_status VARCHAR(20) DEFAULT 'ON_CAMPUS',
    internship_company VARCHAR(200),
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

说明：`teacher_id` 为学生负责老师 ID。老师端审批外出实习和请假申报时，只能查看并审批 `student.teacher_id` 等于当前老师 ID 的学生申请；管理员可查看全部申请。导入学生时只要求 `student_no`、`name`、`college`、`major`、`class_name`；开卡不校验学生资料完整性。学生端进行商品消费、图书借阅、通勤乘车、考勤申报和打卡前，业务层要求 `name`、`gender`、`college`、`major`、`class_name`、`phone` 全部完整。

---

#### teacher 教师表

```sql
CREATE TABLE teacher (
    id BIGSERIAL PRIMARY KEY,
    teacher_no VARCHAR(20) NOT NULL,
    name VARCHAR(50),
    gender VARCHAR(10),
    department VARCHAR(100),
    phone VARCHAR(20),
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

#### college_major 学院专业字典表

```sql
CREATE TABLE college_major (
    id BIGSERIAL PRIMARY KEY,
    college_name VARCHAR(100) NOT NULL,
    major_name VARCHAR(100) NOT NULL,
    status INT DEFAULT 1,
    sort_order INT DEFAULT 0,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

说明：用于维护新增学生、编辑学生、新增教师、编辑教师时可选择的学院和专业。学生表仍保存 `college`、`major` 文本，教师表仍保存 `department` 文本，字典表负责提供下拉选项；管理员可停用或软删除不再使用的学院专业。

---

### 3.2 校园卡管理

#### campus_card

```sql
CREATE TABLE campus_card (
    id BIGSERIAL PRIMARY KEY,
    card_no VARCHAR(50) UNIQUE NOT NULL,
    user_id BIGINT,
    user_type VARCHAR(20),
    status INT DEFAULT 1,
    issue_date DATE,
    expire_date DATE,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

#### card_change_record（卡操作记录）

```sql
CREATE TABLE card_change_record (
    id BIGSERIAL PRIMARY KEY,
    card_id BIGINT,
    operation_type VARCHAR(20),
    operator_id BIGINT,
    remark VARCHAR(200),
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

### 3.3 账户管理

#### account

```sql
CREATE TABLE account (
    id BIGSERIAL PRIMARY KEY,
    card_id BIGINT,
    balance NUMERIC(10,2) DEFAULT 0,
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

#### account_flow（账户流水）

```sql
CREATE TABLE account_flow (
    id BIGSERIAL PRIMARY KEY,
    account_id BIGINT,
    change_type VARCHAR(20),
    amount NUMERIC(10,2),
    balance_after NUMERIC(10,2),
    related_id BIGINT,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

### 3.4 充值管理

#### recharge_record

```sql
CREATE TABLE recharge_record (
    id BIGSERIAL PRIMARY KEY,
    account_id BIGINT,
    amount NUMERIC(10,2),
    recharge_type VARCHAR(20),
    operator_id BIGINT,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

#### recharge_order

```sql
CREATE TABLE recharge_order (
    id BIGSERIAL PRIMARY KEY,
    out_trade_no VARCHAR(64) NOT NULL,
    card_id BIGINT NOT NULL,
    amount NUMERIC(10,2) NOT NULL,
    status VARCHAR(20) DEFAULT 'WAIT_PAY',
    alipay_trade_no VARCHAR(64),
    operator_id BIGINT,
    paid_time TIMESTAMP,
    settled_time TIMESTAMP,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

- `WAIT_PAY`：已创建支付宝沙箱支付订单，等待支付。
- `SETTLED`：支付宝确认支付成功，系统已完成校园卡余额入账。
- `CLOSED`：支付宝交易关闭，不入账。

---

### 3.5 消费管理

#### consume_record

```sql
CREATE TABLE consume_record (
    id BIGSERIAL PRIMARY KEY,
    account_id BIGINT,
    merchant_id BIGINT,
    amount NUMERIC(10,2),
    balance_after NUMERIC(10,2),
    status INT DEFAULT 1,
    consume_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0,
    product_id BIGINT,
    product_name VARCHAR(100),
    quantity INT DEFAULT 1
);
```

---

### 3.6 商户管理

#### merchant_type

```sql
CREATE TABLE merchant_type (
    id BIGSERIAL PRIMARY KEY,
    type_name VARCHAR(50),
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

#### merchant

```sql
CREATE TABLE merchant (
    id BIGSERIAL PRIMARY KEY,
    merchant_name VARCHAR(100),
    type_id BIGINT,
    location VARCHAR(200),
    logo VARCHAR(255),
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

#### product 商品表

```sql
CREATE TABLE product (
    id BIGSERIAL PRIMARY KEY,
    product_name VARCHAR(100) NOT NULL,
    merchant_id BIGINT NOT NULL,
    price NUMERIC(10,2) NOT NULL,
    stock INT DEFAULT 0,
    description VARCHAR(500),
    image VARCHAR(255),
    status SMALLINT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted SMALLINT DEFAULT 0
);
```

说明：商品表用于商户商品维护和学生端商品购买。`consume_record.product_id`、`product_name`、`quantity` 保存商品消费时的商品 ID、商品名称快照和购买数量。

---

## 四、辅助业务模块

---

### 4.1 门禁管理

#### access_point（门禁点表）

```sql
CREATE TABLE access_point (
    id BIGSERIAL PRIMARY KEY,
    name VARCHAR(100),
    location VARCHAR(200),
    latitude DECIMAL(10, 6),
    longitude DECIMAL(10, 6),
    radius INT DEFAULT 50,
    device_id VARCHAR(50),
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

#### access_record（门禁记录表）

```sql
CREATE TABLE access_record (
    id BIGSERIAL PRIMARY KEY,
    card_id BIGINT,
    access_point_id BIGINT,
    direction VARCHAR(10),
    location VARCHAR(100),
    status VARCHAR(20),
    actual_latitude DECIMAL(10, 6),
    actual_longitude DECIMAL(10, 6),
    distance DECIMAL(10, 2),
    access_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    device_info VARCHAR(255),
    is_deleted INT DEFAULT 0
);
```

---

### 4.2 图书借阅

#### book

```sql
CREATE TABLE book (
    id BIGSERIAL PRIMARY KEY,
    book_name VARCHAR(100),
    author VARCHAR(100),
    collection_location VARCHAR(200),
    logo VARCHAR(255),
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

#### borrow_application

```sql
CREATE TABLE borrow_application (
    id BIGSERIAL PRIMARY KEY,
    card_id BIGINT,
    book_id BIGINT,
    borrow_days INT,
    application_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    status INT DEFAULT 1, -- 1: 待审批, 2: 已批准, 3: 已拒绝
    operator_id BIGINT,
    approval_time TIMESTAMP,
    remark VARCHAR(200),
    is_deleted INT DEFAULT 0,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP
);
```

---

#### borrow_record

```sql
CREATE TABLE borrow_record (
    id BIGSERIAL PRIMARY KEY,
    card_id BIGINT,
    book_id BIGINT,
    application_id BIGINT,
    borrow_time TIMESTAMP,
    return_time TIMESTAMP,
    due_time TIMESTAMP,
    status INT DEFAULT 1, -- 1: 借阅中, 2: 已归还, 3: 超期
    is_deleted INT DEFAULT 0
);
```

---

### 4.3 考勤管理

#### attendance_location（打卡位置表）

```sql
CREATE TABLE attendance_location (
    id BIGSERIAL PRIMARY KEY,
    teacher_id BIGINT,
    location_name VARCHAR(200),
    location VARCHAR(200),
    latitude DECIMAL(10, 6),
    longitude DECIMAL(10, 6),
    radius INT DEFAULT 50,
    start_time TIMESTAMP,
    end_time TIMESTAMP,
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

#### attendance_record（考勤记录表）

```sql
CREATE TABLE attendance_record (
    id BIGSERIAL PRIMARY KEY,
    card_id BIGINT,
    location_id BIGINT,
    status VARCHAR(20),
    actual_location VARCHAR(200),
    actual_latitude DECIMAL(10, 6),
    actual_longitude DECIMAL(10, 6),
    device_info VARCHAR(255),
    attendance_type VARCHAR(30) DEFAULT 'CAMPUS_LOCATION',
    internship_company VARCHAR(200),
    internship_log TEXT,
    internship_log_date DATE,
    leave_application_id BIGINT,
    leave_reason VARCHAR(500),
    leave_start_date DATE,
    leave_end_date DATE,
    record_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

说明：`attendance_type` 区分校内打卡、校外位置、实习日志和请假记录；实习日志、请假原因和请假日期等字段已直接在 `init.sql` 中定义。

#### attendance_application（考勤申报表）

```sql
CREATE TABLE attendance_application (
    id BIGSERIAL PRIMARY KEY,
    student_id BIGINT,
    card_id BIGINT,
    application_type VARCHAR(30),
    internship_company VARCHAR(200),
    reason TEXT,
    start_date DATE,
    end_date DATE,
    status VARCHAR(20) DEFAULT 'PENDING',
    reviewer_id BIGINT,
    review_time TIMESTAMP,
    review_remark VARCHAR(1000),
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

说明：`reason` 保存申报原因、说明或富文本资料内容，可能包含图片 `<img>` 与附件链接 `<a>`；该字段在 `init.sql` 中直接定义为 `TEXT`。

---

### 4.4 通勤车管理

#### commute_route（通勤车线路表）

```sql
CREATE TABLE commute_route (
    id BIGSERIAL PRIMARY KEY,
    route_name VARCHAR(100) NOT NULL,
    start_station VARCHAR(100),
    end_station VARCHAR(100),
    total_distance DECIMAL(10, 2),
    total_time INT, -- 全程时间（分钟）
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

#### commute_vehicle（通勤车车辆表）

```sql
CREATE TABLE commute_vehicle (
    id BIGSERIAL PRIMARY KEY,
    plate_number VARCHAR(20) UNIQUE NOT NULL,
    vehicle_type VARCHAR(50),
    seat_count INT,
    status INT DEFAULT 1, -- 1: 正常, 2: 维护, 3: 停用
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

#### commute_station（通勤车站点表）

```sql
CREATE TABLE commute_station (
    id BIGSERIAL PRIMARY KEY,
    station_name VARCHAR(100) NOT NULL,
    location VARCHAR(200),
    latitude DECIMAL(10, 6),
    longitude DECIMAL(10, 6),
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

#### commute_schedule（通勤车时刻表）

```sql
CREATE TABLE commute_schedule (
    id BIGSERIAL PRIMARY KEY,
    route_id BIGINT,
    vehicle_id BIGINT,
    departure_time TIME NOT NULL,
    frequency VARCHAR(50), -- 班次频率，如：每天、工作日、周末
    start_date DATE,
    end_date DATE,
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    update_time TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

#### commute_record（通勤车记录表）

```sql
CREATE TABLE commute_record (
    id BIGSERIAL PRIMARY KEY,
    card_id BIGINT,
    route_id BIGINT,
    vehicle_id BIGINT,
    schedule_id BIGINT,
    seat_number VARCHAR(10),
    ride_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    status INT DEFAULT 1,
    is_deleted INT DEFAULT 0
);
```

---

## 五、系统管理模块

---

### 5.1 管理员

```sql
CREATE TABLE admin_user (
    id BIGSERIAL PRIMARY KEY,
    username VARCHAR(50),
    password VARCHAR(100),
    role VARCHAR(20),
    status INT DEFAULT 1,
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

### 5.2 操作日志

```sql
CREATE TABLE operation_log (
    id BIGSERIAL PRIMARY KEY,
    operator_id BIGINT,
    operation_type VARCHAR(50),
    target_table VARCHAR(50),
    target_id BIGINT,
    content VARCHAR(500),
    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    is_deleted INT DEFAULT 0
);
```

---

## 六、唯一约束与索引设计

### 6.1 唯一约束

```sql
ALTER TABLE account ADD CONSTRAINT uk_account_card UNIQUE(card_id);
CREATE UNIQUE INDEX uk_student_no_active ON student(student_no) WHERE is_deleted = 0;
CREATE UNIQUE INDEX uk_teacher_no_active ON teacher(teacher_no) WHERE is_deleted = 0;
ALTER TABLE merchant_type ADD CONSTRAINT uk_merchant_type_name UNIQUE(type_name);
ALTER TABLE merchant ADD CONSTRAINT uk_merchant_name UNIQUE(merchant_name);
CREATE UNIQUE INDEX uk_admin_username_active ON admin_user(username) WHERE is_deleted = 0;
CREATE UNIQUE INDEX uk_college_major_active ON college_major(college_name, major_name) WHERE is_deleted = 0;
CREATE UNIQUE INDEX idx_recharge_order_out_trade_no ON recharge_order(out_trade_no);
```

说明：

- `account.card_id` 唯一，保证一张校园卡只对应一个账户。
- `student_no`、`teacher_no`、`admin_user.username`、`college_major` 使用未删除数据唯一索引，兼容软删除后的重新录入。
- `merchant_type.type_name`、`merchant.merchant_name` 使用唯一约束，避免基础数据重复。
- `recharge_order.out_trade_no` 唯一，保证支付宝商户订单号幂等。

### 6.2 普通索引

```sql
-- 校园卡相关索引
CREATE INDEX idx_card_no ON campus_card(card_no);
CREATE INDEX idx_campus_card_user_id ON campus_card(user_id);
CREATE INDEX idx_campus_card_status ON campus_card(status);

-- 账户相关索引
CREATE INDEX idx_account_card_id ON account(card_id);
CREATE INDEX idx_account_status ON account(status);

-- 充值记录索引
CREATE INDEX idx_recharge_account_id ON recharge_record(account_id);
CREATE INDEX idx_recharge_time ON recharge_record(create_time);
CREATE INDEX idx_recharge_order_card_id ON recharge_order(card_id);
CREATE INDEX idx_recharge_order_status ON recharge_order(status);

-- 消费记录索引
CREATE INDEX idx_consume_account_id ON consume_record(account_id);
CREATE INDEX idx_consume_merchant_id ON consume_record(merchant_id);
CREATE INDEX idx_consume_product_id ON consume_record(product_id);
CREATE INDEX idx_consume_time ON consume_record(consume_time);
CREATE INDEX idx_consume_status ON consume_record(status);

-- 账户流水索引
CREATE INDEX idx_account_flow_account_id ON account_flow(account_id);
CREATE INDEX idx_account_flow_change_type ON account_flow(change_type);
CREATE INDEX idx_account_flow_time ON account_flow(create_time);

-- 用户相关索引
CREATE INDEX idx_student_no ON student(student_no);
CREATE INDEX idx_student_teacher_id ON student(teacher_id);
CREATE INDEX idx_student_attendance_mode ON student(attendance_mode);
CREATE INDEX idx_teacher_no ON teacher(teacher_no);
CREATE INDEX idx_college_major_college ON college_major(college_name);
CREATE INDEX idx_college_major_status ON college_major(status);

-- 商户相关索引
CREATE INDEX idx_merchant_type_id ON merchant(type_id);
CREATE INDEX idx_merchant_status ON merchant(status);
CREATE INDEX idx_product_merchant_id ON product(merchant_id);
CREATE INDEX idx_product_name ON product(product_name);
CREATE INDEX idx_product_status ON product(status);

-- 辅助模块索引
CREATE INDEX idx_access_card_id ON access_record(card_id);
CREATE INDEX idx_access_time ON access_record(access_time);
CREATE INDEX idx_access_point_id ON access_record(access_point_id);
CREATE INDEX idx_access_status ON access_record(status);
CREATE INDEX idx_access_point_status ON access_point(status);
CREATE INDEX idx_access_point_device_id ON access_point(device_id);
CREATE INDEX idx_borrow_card_id ON borrow_record(card_id);
CREATE INDEX idx_borrow_book_id ON borrow_record(book_id);
CREATE INDEX idx_attendance_location_teacher_id ON attendance_location(teacher_id);
CREATE INDEX idx_attendance_location_status ON attendance_location(status);
CREATE INDEX idx_attendance_location_time ON attendance_location(start_time);
CREATE INDEX idx_attendance_card_id ON attendance_record(card_id);
CREATE INDEX idx_attendance_time ON attendance_record(record_time);
CREATE INDEX idx_attendance_type ON attendance_record(attendance_type);
CREATE INDEX idx_attendance_internship_log_date ON attendance_record(internship_log_date);
-- 通勤车相关索引
CREATE INDEX idx_commute_route_status ON commute_route(status);
CREATE INDEX idx_commute_vehicle_plate ON commute_vehicle(plate_number);
CREATE INDEX idx_commute_vehicle_status ON commute_vehicle(status);
CREATE INDEX idx_commute_station_name ON commute_station(station_name);
CREATE INDEX idx_commute_station_status ON commute_station(status);
CREATE INDEX idx_commute_schedule_route ON commute_schedule(route_id);
CREATE INDEX idx_commute_schedule_vehicle ON commute_schedule(vehicle_id);
CREATE INDEX idx_commute_schedule_departure ON commute_schedule(departure_time);
CREATE INDEX idx_commute_schedule_status ON commute_schedule(status);
CREATE INDEX idx_commute_record_card_id ON commute_record(card_id);
CREATE INDEX idx_commute_record_route_id ON commute_record(route_id);
CREATE INDEX idx_commute_record_vehicle_id ON commute_record(vehicle_id);
CREATE INDEX idx_commute_record_time ON commute_record(ride_time);

-- 系统日志索引
CREATE INDEX idx_operation_log_operator_id ON operation_log(operator_id);
CREATE INDEX idx_operation_log_type ON operation_log(operation_type);
CREATE INDEX idx_operation_log_time ON operation_log(create_time);
```

---

## 七、初始化数据

`init.sql` 包含两类基础数据：

1. 学院专业字典：向 `college_major` 写入材料科学与工程学院、机电工程学院、计算机与人工智能学院等学院及专业，用于学生、教师维护页面的学院专业下拉选项。
2. 通勤车测试数据：向 `commute_station`、`commute_vehicle`、`commute_route`、`commute_schedule` 写入站点、车辆、线路和班次，用于通勤车模块初始化展示与联调。

`init.sql` 当前只保留必要基础数据；学生、教师、校园卡、账户、商户、图书、消费记录、充值记录等业务数据由系统功能在运行过程中维护。

---

## 八、设计总结

本系统数据库特点：

1. 模块划分清晰（核心 + 辅助 + 系统）
2. 采用逻辑外键，提高扩展性
3. 所有数据支持软删除
4. 关键操作全部留痕（流水 + 日志）
5. 通过唯一约束和业务索引保证常用查询、状态筛选和交易流水追踪的效率
